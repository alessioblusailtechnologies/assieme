import { FormatoGenerazione, TemplateOutput } from '@core/models';
 ...
 * formato e, senza predefinito, il layout di piattaforma.
 */
export type SceltaEsporta = { templateId: string } | { formato: FormatoGenerazione };
...
/**
 * Le voci del menù: per ogni formato i template dell'agenzia (il predefinito
 * segnalato), e il layout di piattaforma per i formati senza template. PPTX
 * non si genera ancora: un template PPTX non compare.
 */
...
export function nomeFileEsportazione(nome: string, formato: FormatoGenerazione): string {
  return `${nome.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}.${formato}`;
}
