...
**Stato**: **chiuso** (12/09/2026): l'albero è smontato, le API dei clienti ci sono, l'ingestion intesta ed etichetta da sé, la sezione Clienti si vede, l'archivio si lavora in blocco, la chat col cliente si apre dalla sua scheda e il motore naviga per cliente. Tutte e sette le fasi fatte.
...
- **La ricarica dell'archivio demo non si fa** (deciso il 12/09/2026): quei 22 documenti privati stanno in un ambiente di prova, e rifarli costerebbe chiamate al modello per un archivio che nessuno userà. Restano come sono, i più senza cliente, e quel tenant non va letto come misura dell'ingestion nuova. Su un archivio vero il giro si fa con `tools/rilavora-ingestion.ts`, che rilavora un id in qualunque stato.
