...
#### Revisione dei template — **fatta** (25/08/2026), su indicazione del committente

Il modello «libreria di piattaforma + predefinito per tipologia» era opaco: quattro righe senza file che l'utente vedeva ma non poteva toccare, e un solo template per tipologia. Il committente vuole una cosa più semplice: **un template è un documento dell'agenzia caricato dalle Impostazioni, quanti ne vuole, anche più d'uno per formato, ognuno col nome con cui lo richiama in chat** («esporta con Proposta breve»), con un **predefinito per formato**. Decisioni:

- **Migrazione `20260828090000_template_per_formato.sql`**: via le righe di piattaforma (`tenant_id`/`path_file` ora `not null`), via `tipologia_libreria` e la tabella `template_predefiniti`; `predefinito boolean` sulla riga con indice unico parziale `(tenant_id, formato) where predefinito`; policy di update. Gli agenti che puntavano a `tpl-001…004` restano senza template (FK `set null`). **Il layout di piattaforma non è più in catalogo**: è il fallback nel codice quando per il formato chiesto non c'è un template — `risolviTemplate` in `src/generazione/catalogo.ts` (le letture di catalogo/identità sono lì, condivise da API e worker; `api/template/rotte.ts` le riesporta).
- **Contratto**: `TemplateOutput { id, nome, formato, descrizione, predefinito }` (spariti `personalizzato` e `tipologiaPredefinita`); `PATCH /api/template/:id { nome?, predefinito? }` (assegnare il predefinito lo toglie a chi lo portava, risponde con l'elenco); DELETE su tutti (404 se ignoto, niente più 409); il primo template di un formato ne è il predefinito. **Esportazioni** (chat, tabelle): corpo `{ templateId } | { formato }` — col solo formato vale il predefinito, senza predefinito il layout di piattaforma (nome file `documento-velia.<fmt>`). Mock allineato (`risolviTemplate` in `mocks/impostazioni.mjs`, fixture con soli template dell'agenzia).
- **I segnaposto sono facoltativi** (chiesto dal committente: «posso caricare un template con logo e pagina bianca»): un DOCX/XLSX senza `{{contenuto}}` è una carta intestata — `completaSegnapostoDocx` aggiunge in coda al corpo (prima del `sectPr` finale) titolo, contenuto e fonti mancanti, e l'XLSX li scrive sotto l'ultima riga del primo foglio; poi il riempimento è quello di sempre. L'upload rifiuta solo ciò che non si apre (via il 400 `SEGNAPOSTO_MANCANTI`).
- **PDF resta carta intestata** (prima pagina come sfondo, impaginazione nel codice): un PDF non ha segnaposto. La fedeltà totale in PDF (riempire il DOCX e convertirlo con LibreOffice nel worker: ~400 MB in più nell'immagine) è rimandata finché non serve davvero — scelta del committente («andiamo con la 1»).
- **Richiamo dalla chat**: il motore ha il tool MCP in-process `genera_documento` (`src/worker/motore/strumenti.ts`, `createSdkMcpServer` dell'Agent SDK; in `allowedTools`, non in `tools` che è il set dei tool integrati). Il prompt di sistema elenca i template dell'agenzia per nome e dice quando usare lo strumento (solo su richiesta esplicita di un file). Il tool risolve il template dal nome detto dall'utente (esatto, o contenuto se univoco; ambiguo/ignoto → esito d'errore che elenca i disponibili), genera con la stessa macchina delle esportazioni, mette il file in `tenant/<tid>/generati/<uuid>.<fmt>` ed emette l'evento di stream **`documento`** (additivo); il messaggio salva l'elenco in `velia.messaggi.documenti` (jsonb, migrazione `20260828100000`) e `GET /api/conversazioni/:id/documenti/:did` lo serve (visibilità via RLS della conversazione); una risposta bocciata cancella i file. Attività mostrata: «Preparo il documento «…»». FE: chip da scaricare sotto la risposta (`bolla-messaggio`), menù di esportazione per formato con «Documento VELIA» dove manca un template (`shared/esportazione/scelte-esportazione.ts`).
- **Collaudo col motore vero** (`tools/collaudo-documento.ts`): domanda con «generami un documento in pdf» → 8 turni, 91 s, 0,41 $, 11 citazioni verificate, PDF di 29 KB col layout di piattaforma. Il modello ha chiuso con la riga «documento pronto» ripetuta due volte: descrizione del tool irrigidita («UNA sola riga»), da ricollaudare al prossimo giro.
- Test: `test/strumenti.spec.ts` (6: scelta del template, etichetta, prompt), `template.spec.ts` (8), `integrazione-template.spec.ts` (11, riscritta: libreria vuota, due DOCX dello stesso formato, predefinito per formato e rinomina, esporta per formato e per id); `integrazione-agenti` carica un proprio DOCX di collaudo; `integrazione-tabelle` esporta per formato. FE 123/123.

#### Esportazione elaborata — **fatta** (25/08/2026), sandbox documentale su Fly.io

Il committente vuole per i documenti «la stessa potenza di Claude Code»: dare un documento anche popolato e ottenerne uno uguale, HTML→PDF, Excel con formule. Deciso: **due vie coi suoi nomi** — **Esporta subito** (la via deterministica di prima, istantanea e gratis: tool `esporta_subito`, pulsante col menù per formato) ed **Esportazione elaborata** (sessione agentica in sandbox: tool `esportazione_elaborata`, pulsante col modulo template/formato/istruzioni). Decisioni da sapere:

- **La sandbox** (`be-node/sandbox/`): immagine Debian con Python (python-docx, openpyxl, docxtpl, pypdf, reportlab, matplotlib…), Node (docx, exceljs, pdf-lib, docxtemplater), LibreOffice, Chromium (`chromium-headless`), poppler (`pdftoppm`), e l'agente HTTP `server.mjs` (esegui / file / elenco / archivio, token per job in `x-velia-token`). 1,8 GB. Nessun segreto dentro: riceve i file, lavora, restituisce i file. Due avviatori (`src/worker/sandbox/sandbox.ts`): **Docker** in locale (Rancher Desktop; `-p 127.0.0.1:0:8080`, 2 CPU/2 GB) e **Fly Machines** in produzione (`POST /v1/apps/velia-sandbox/machines`, `auto_destroy`, raggiunta via `https://velia-sandbox.fly.dev` + intestazione `fly-force-instance-id`; app creata via API, IPv4 condiviso 66.241.124.176; immagine su `registry.fly.io/velia-sandbox:latest`). Collaudo Fly vero: Machine su in 40 s al primo pull, comandi e Chromium→PDF ok, distrutta a fine job. Railway resta per API e worker (DEPLOY.md §2b).
- **Claude Code DENTRO la sandbox** (rivisto lo stesso giorno su richiesta del committente: «la stessa potenza di Claude Code», non un Bash mascherato). Il runner `sandbox/server.mjs` avvia `query()` dell'Agent SDK con cwd `/lavoro`, strumenti nativi (Read, Write, Edit, Bash, Glob, Grep, Skill, TodoWrite; vietati WebFetch/WebSearch/Task), `permissionMode: bypassPermissions`, le **skill Anthropic open source per docx/xlsx/pptx/pdf** (`anthropics/skills`, copiate in `/lavoro/.claude/skills` e caricate con `settingSources: ['project']`) e un tool MCP locale `consegna`. Lo stream della sessione torna al worker come SSE (`POST /sessione`: attivita | testo | consegna | fine) e il worker (`src/worker/sandbox/esportazione.ts`) inoltra le attività al FE con etichette da utente (`etichettaSandbox`: la `description` dei Bash, i titoli dei documenti, «Controllo la pagina renderizzata»), a ogni `consegna` ritira il file (Storage `tenant/<tid>/generati/`, evento `documento`), a `fine` prende costo e token dal `result` dell'SDK. `DELETE /sessione` annulla.
- **Chiave e rete, per costruzione e non per filtro** (dopo due tentativi falliti: l'allowlist per IP non regge perché `api.anthropic.com` sta dietro Cloudflare e cambia indirizzi; il match `-m owner` di iptables non esiste nel kernel di Fly). Soluzione in `avvio.sh`: un **network namespace `lavoro`** senza rotta di default, collegato al namespace principale da un solo veth (`10.200.0.1` ↔ `10.200.0.2`); **Claude Code e i comandi del modello girano lì dentro** come utente `lavoro` (wrapper `claude-lavoro` via `pathToClaudeCodeExecutable`: `ip netns exec lavoro setpriv --reuid=lavoro …`; anche `/esegui` passa da lì) e possono raggiungere SOLO il **proxy della chiave** `proxy.mjs` (utente di sistema `proxy`, namespace principale, `10.200.0.1:8787` → `api.anthropic.com`). La chiave (`ANTHROPIC_API_KEY_SANDBOX`, dedicata con tetto di spesa; in sviluppo ricade su `ANTHROPIC_API_KEY`) entra solo nel proxy; il runner (root, namespace principale) e il modello non la vedono. Verificato in Docker e su Fly: dal Bash del modello `echo $ANTHROPIC_API_KEY` → vuoto, `curl google` → bloccato, `curl api.anthropic.com` diretto → bloccato, proxy → raggiunto. In Docker servono `--cap-add NET_ADMIN --cap-add SYS_ADMIN` (namespace); nella Machine Fly siamo root in una VM. Trappole trovate: `authorization: undefined` nel proxy faceva crashare il runner; `su -c '…' -- "$@"` mangia il primo argomento della CLI (`--input-format` senza `--output-format`): per questo `setpriv`; le variabili davanti a `cd && exec node` valgono solo per `cd`.
- Il prompt (`istruzioni.ts`) impone il ciclo «capisci il template (con le skill), produci, converti, renderizza in PNG, guardale con Read, correggi, consegna» e chiede la `description` dei Bash in italiano per l'utente. Tetti: `SANDBOX_MAX_TURNI` 60, `SANDBOX_BUDGET_USD` 4. Ogni ritocco al prompt si ricollauda con `tools/collaudo-elaborata.ts`.
- **Gli ingressi**: dal pulsante, `POST …/messaggi` con `esportazione: { formato, templateId?, messaggioId?, istruzioni? }` (contratto additivo; il job `interrogazione` col campo produce un documento, non una risposta: niente blocco citazioni, messaggio assistente = nota del motore documentale + `documenti`); a parole, il tool `esportazione_elaborata` della chat esegue la stessa funzione in linea e i file consegnati diventano documenti della risposta. File orfani cancellati se la risposta non si salva. Config: `SANDBOX_AVVIATORE` (docker|fly, vuoto = non disponibile), `SANDBOX_IMMAGINE`, `FLY_API_TOKEN`, `FLY_APP_SANDBOX`, `FLY_REGIONE`.
- **Collaudo col motore vero e la sandbox Docker** (`tools/collaudo-elaborata.ts`, tenant demo senza template): «scheda di due pagine per il cliente Rossi sulle garanzie Incendio e Furto di Km&Servizi, con tabella e fonti» → il modello ha letto CGA, DIP e DIP Aggiuntivo, scritto HTML+CSS, stampato con Chromium, reso le pagine in PNG e le ha guardate, **compattato il testo per stare in due pagine in tre giri**, consegnato un PDF di 115 KB davvero professionale (testata, tabella, fonti con articoli e pagine, nota di edizione). 32 turni, 326 s, 1,60 $. È il livello di qualità voluto; il costo è quello di una sessione lunga.
- **Collaudo con Claude Code nella sandbox** (DOCX, Opus, Docker locale): «lettera di una pagina in Word per il cliente Rossi sulle garanzie Incendio e Furto» → ha letto CGA e DIP, consultato la skill docx, generato il DOCX, reso in PNG e guardato, compattato due volte per stare in una pagina, consegnato (581 parole, piè di pagina, intestazione ricavata da un preventivo in archivio perché l'identità visiva del demo è vuota, firma lasciata come campo da compilare). 34 turni, 269 s, 1,44 $. Bug trovati e chiusi al collaudo: `dentro()` del runner trattava i path assoluti come relativi (`/lavoro/lavoro/…`: la consegna falliva e il modello riprovava sei volte); il messaggio finale accumulava i commenti intermedi (ora è il `result` dell'SDK).
- Test: `test/sandbox.spec.ts` (prompt + giro vero su Docker se l'immagine c'è: esegui, file, archivio, HTML→PDF→PNG, timeout, path fuori radice), `strumenti.spec.ts` aggiornato ai due tool.
- Rimandati: upload di allegati/template DOCX/XLSX/HTML (sì del committente, prossimo passo); rete nella sandbox (nessuna, per scelta); il web con allowlist.

### ✅ Fase 5 — Tabelle di analisi — **completata** (24/08/2026) · RF-C-11…C-15

`/api/tabelle` risponde dal backend (proxy al 3002); il mock resta per la demo, contratto identico. **Collaudato col motore vero** (`tools/collaudo-tabelle.ts`): una riga sulle Condizioni UnipolSai Km&Servizi reali, 3 criteri (2 predefiniti + 1 personalizzato) → 3 celle valutate, citazioni **tutte verificate** contro file e pagine (175, 83, 176), 14 turni, 83,5 s, 0,34 $ con Opus. Decisioni da sapere:

- **Lo schema è per chiave, non per blob**: `tabelle` + `tabelle_colonne` + `tabelle_righe` (chiave `tabella, documento`; etichetta denormalizzata: un documento eliminato dagli archivi non porta via la riga) + `tabelle_celle` (chiave `tabella, documento, colonna`, FK a cascata su righe e colonne). È ciò che rende le **mutazioni a generazione in corso** banali: aggiungere una colonna inserisce celle `in-attesa`, togliere una riga le cancella in cascata — il job non va avvisato, riconcilia da sé. `velia.tabelle_criteri` è dato di piattaforma (stessi id del mock), seminato in migrazione.
- **Il popolamento è «per gruppi per documento»**, la scelta che il piano lasciava alla misura: una sessione del motore per riga estrae TUTTE le colonne in attesa di quel documento (una per cella moltiplicherebbe workspace e prompt). Il gestore (`worker/tabelle/gestore.ts`) a ogni giro rilegge dal db la prossima riga con celle in attesa — è questo il punto di riconciliazione — e chiude con `stato = 'completa'` solo se non ne restano (guardia `not exists` contro la mutazione arrivata fra ricerca e chiusura). I consumi si registrano **per riga**: sono i numeri con cui si deciderà `MODELLO_TABELLE` (default = `MODELLO_MOTORE`; a ~0,34 $/documento con Opus, Sonnet è il candidato quando i volumi crescono).
- **Il prompt è dedicato** (`worker/tabelle/estrazione.ts`, `PROMPT_ESTRAZIONE`): stesso mondo del motore chat (workspace, ancore `[pag. N]`, tre tool di lettura), ma un solo output ammesso — il blocco ```` ```velia-celle ```` con una voce per colonna richiesta (id esatti nel prompt; le predefinite portano la descrizione del criterio come guida). Può seguire i richiami espliciti del set (preventivo → Condizioni) e citare quelli.
- **RF-C-12 è più dura della chat**: un esito «presente» senza una citazione che regge (file reale della workspace, pagina entro l'ultima citabile, mai un INDICE) **non boccia la tabella** — diventa `non-determinabile` con motivo, perché una cella che afferma senza fonte non deve esistere ma un'allucinazione su una cella non deve costare le altre 40. Una colonna saltata dal modello idem; budget o errore su una riga marcano le sue celle e si passa alla successiva. Solo il fallimento dell'intero job (non ritentabile, o ultimo tentativo) porta la tabella a `stato = 'errore'`: un guasto transitorio la lascia `in-generazione` e il retry del ciclo la finisce — il polling del FE non vede mai un errore che sta per rientrare.
- **Visibilità = conversazioni (RF-C-15)**: RLS — l'autore, e i colleghi del tenant se `condivisa` (in sola lettura: le mutazioni sono 403 `Solo chi ha creato la tabella può modificarla`, prima dal codice poi dalla RLS). «Duplica» è di chiunque veda: copia con colonne a id nuovi (le celle li referenziano: si rimappano), celle pronte comprese, `Copia di <titolo>`, non condivisa, dell'utente che chiede; se l'originale era in generazione la copia riparte da dove era (job suo).
- **Contratto del mock, alla lettera**: 400 `TABELLA_VUOTA`/`COLONNA_VUOTA`, 404 `Documento «id» inesistente.`, 409 `NON_PRONTO` sui non elaborati, 404 `NON_TROVATA` sulla tabella (id malformato compreso: mai un errore SQL), titolo predefinito («Confronto <ramo>» se il ramo è uno, altrimenti dalla data), etichette di riga pronte (compagnia — prodotto, con l'edizione se storica; titolo per i privati), elenco `Paginato` a pagina unica per `aggiornataIl` desc coi conteggi, criteri filtrati sui rami dei documenti (`GET /api/tabelle/criteri?documenti=…`), rimozioni idempotenti che rispondono con la tabella intera.
- **L'esportazione è la Fase 4 al lavoro** (RF-C-14): la tabella diventa Markdown (`testoTabella`, celle su colonne vere nei compositori; `—` per le celle in attesa, `Non presente`/`Non determinabile` dichiarati) + fonti nella forma del mock, e `generaDocumento` fa il resto — XLSX su colonne reali, DOCX/PDF impaginati, template del tenant compresi. Il nome del file viene dal **titolo della tabella** (regola del mock), non dal template.
- Job `tipo = 'tabella'` nella coda unica (constraint aggiornato); accodato a creazione, aggiunta documenti/colonne e duplica in corso — sempre, anche se un job gira già: è idempotente, un doppione trova zero celle e chiude. «Interroga in chat» non ha una rotta: è il FE che apre una conversazione con gli stessi documenti (RF-C-13, già nel contratto).
- Migrazione `20260825190000_tabelle.sql` applicata con `tools/applica-migrazione.mjs`. Test: `test/tabelle.spec.ts` (11: schemi, blocco `velia-celle`, regola RF-C-12, Markdown d'esportazione, rotte prima del db) e `test/integrazione-tabelle.spec.ts` (9, tenant di collaudo, motore finto pilotato dal prompt: creazione → job → celle con citazioni validate, colonna aggiunta che riapre la generazione, valore senza fonte scartato, condivisione/duplica, esportazione XLSX riletta con exceljs). Suite completa 136/136, due volte. Ogni ritocco a `PROMPT_ESTRAZIONE` si ricollauda con `tools/collaudo-tabelle.ts` (centesimi con Sonnet, ~0,35 $ con Opus).
- Rimandati con motivo: il lookup strutturato del doc motore §7 (lo schema delle celle è già per chiave documento×criterio: la base c'è); l'annullamento esplicito di una generazione (nessun contratto FE); pesare gruppi più larghi di un documento per sessione (prima servono i numeri di `consumi` su tabelle vere).

### ✅ Fase 6 — Impostazioni complete — **completata** (24/08/2026) · RF-D-01…D-04, D-06/07, D-14/16, utenti

Tutto il Modulo D risponde dal backend: modelli, istruzioni (regole e documenti di riferimento), storico unico, utenti. Proxy: `/api/modelli`, `/api/istruzioni`, `/api/impostazioni`, `/api/utenti` → 3002; il mock resta per la demo. Decisioni da sapere:

- **La scelta del modello è reale e arriva fino al job (RF-D-02)**: `velia.tenant.modello_motore` (null = default di piattaforma, `MODELLO_MOTORE` — Claude Opus 5) viaggia in `RichiestaMotore.modello` e i gestori di chat e tabelle la leggono a ogni sessione; `GET /api/modelli/attivo` la deriva — la scheda non può mentire su cosa gira. Gli Anthropic (Opus 5, Sonnet 5, Haiku 4.5) sono selezionabili; GPT-5.2 e Mistral restano schede informative non disponibili (409 `NON_DISPONIBILE`) finché l'integrazione multi-provider non esiste — l'interfaccia `Motore` è già il punto d'innesto. **La colonna si scrive con la connessione di sistema** dopo la guardia da amministratore: la riga di tenant porta i limiti di piano, una policy di update la consegnerebbe a chiunque via PostgREST.
- **Regole (RF-D-04/06/08)**: CRUD su `velia.istruzioni` (le policy di scrittura, promesse in Fase 3, aprono qui — tenant nella RLS, amministratore nel codice), ambiti come unione discriminata verificata sulle tassonomie (400 su ramo/compagnia inesistenti), 400 `REGOLA_VUOTA` e 404 `NON_TROVATA` del mock; lo storico distingue attivazione/disattivazione da modifica, come il mock.
- **Un documento di riferimento È un documento privato con un ruolo (RF-D-14/15)** — la decisione che tiene in piedi le citazioni: il contratto FE (`Citazione.archivio`) non ha un archivio «riferimento», quindi anche il caricato dalle Impostazioni diventa una riga di `documenti` (archivio `privato`, stessa pipeline, stesso visualizzatore, flag già addosso). Il **governo** del ruolo — origine `promosso`/`caricato`, ambito, attivazione — sta in `velia.riferimenti` (una voce per documento; la promozione RF-B-09 la crea, la demozione la toglie, migrazione con backfill dei promossi di ieri). **È il governo a decidere cosa entra nel DNA**: `caricaDna` legge le voci attive con ambito pertinente, non più il solo flag. Togliere il ruolo: il promosso resta in archivio, il caricato sparisce con riga e file (RNF-03).
- **RF-D-16, il peso è il Markdown**: l'ingestion misura il convertito (`documenti.dimensione_md_byte`) ed è quello che `dimensioneByte` espone — il contesto permanente si paga in testo, non in byte di PDF (che restano la stima finché la conversione non è passata).
- **Storico unico (RF-D-07)**: `GET /api/impostazioni/storico?oggetti=…`, join su `utenti` per il nome, le 50 più recenti — alimentato dalla Fase 4 in poi su template, identità, modello, regole e riferimenti.
- **Utenti (RF-D-01)**: elenco (da amministratore, come il mock), invito che crea l'utente **su Supabase Auth** con `app_metadata` (tenant e ruolo, che l'utente non può scrivere) + profilo `invitato` → attivo al primo accesso (già Fase 1); se l'email d'invito non parte (SMTP), la creazione avviene comunque — meglio un invito senza email che nessun invito. Cambio ruolo scritto **anche in Auth** (vive nel token; quello in corso resta valido fino a scadenza, ~1 h). Sospensione, mai eliminazione; 409 `SE_STESSO` su sé stessi, 409 `EMAIL_ESISTENTE` sui doppi.
- Migrazione `20260826090000_impostazioni.sql`. Test: `test/impostazioni.spec.ts` (8) e `test/modelli.spec.ts` aggiornato; integrazione `test/integrazione-impostazioni.spec.ts` (6: regole+storico, due origini col peso md, governo→DNA, demozione nelle due nature, **invito vero su Auth** con pulizia, filtro storico) e `test/integrazione-modelli.spec.ts` (scelta scritta sul tenant e riflessa dall'attivo). Suite completa 156/156; provato dal vivo dal 4200 sul tenant demo.
- Rimandati con motivo: RF-D-09 (prova di una regola — priorità C, dopo-lancio, come nel FE); la seconda implementazione provider (l'interfaccia c'è, GPT/Mistral si alzano quando esisterà); revoca immediata dei token alla sospensione (oggi vale dalla scadenza del token, ~1 h — servirebbe una blacklist che il pilota non giustifica ancora).

### ✅ Fase 7 — Agenti — **completata** (24/08/2026) · RF-E-01…E-13

`/api/agenti` risponde dal backend (proxy al 3002); il mock resta per la demo. **Collaudato col motore vero** (`tools/collaudo-agente.ts`): istruzioni realistiche («garanzia cristalli: massimale, franchigia, esclusioni, giudizio per l'intermediario») sulle Condizioni UnipolSai reali → esito da intermediario con **11 citazioni tutte verificate**, 8 turni, 68 s, 0,27 $ con Opus, zero avvisi. Decisioni da sapere:

- **L'esecuzione è lo stesso job d'interrogazione con un ingresso diverso** (job `tipo = 'agente'`, `worker/agenti/gestore.ts`): stessa workspace, stesso prompt di sistema con DNA (le regole dell'agenzia valgono anche per gli agenti), stessa validazione `velia-citazioni`. Il prompt utente (`promptAgente`) porta istruzioni del task, parametri dell'avvio e le **fonti risolte ADESSO** — RF-E-10: «tutto il ramo auto» è l'archivio di oggi (`risolviFonti`: documento puntuale, selezione per archivio/ramo/compagnia, `soloPreferiti` = i preferiti di chi ha creato l'agente, `documenti-riferimento` = le voci ATTIVE del governo di Fase 6). Nel prompt le fonti si elencano fino a 30, poi si dichiara il resto. Modello del tenant (RF-D-02) rispettato anche qui.
- **RF-E-08 come la chat**: un esito che cita passaggi non verificabili è un'esecuzione `fallita` con motivo (`ErroreNonRitentabile`: un'allucinazione non si ritenta, tentativi = 1); il budget raggiunto è un esito parziale dichiarato, non un fallimento. `caricaDna` ora accetta l'utente null (esecuzioni pianificate: niente ricordi personali) — il collaudo ha stanato il cast uuid su stringa vuota.
- **Il retry si racconta (RF-E-11)**: il log dell'esecuzione (jsonb, aggiornato passo per passo: il polling lo vede crescere) segna avvio (con chi, per le manuali), parametri, fonti raccolte, interrogazione, «Nuovo tentativo (N di 3)», esito; il fallimento persistente arriva alla terza consegna con l'errore «per tre tentativi consecutivi». **Fix nel ciclo**: `job.tentativi` ora è il tentativo IN CORSO (la riga era letta prima dell'aggiornamento — off-by-one che toccava anche il gestore tabelle).
- **La pianificazione è pg_cron che accoda al tick**: `velia.prossimo_tick()` calcola l'occorrenza successiva in ora italiana (giornaliera/settimanale/mensile, colonne `pian_*` su `agenti`), l'API la ricalcola a ogni scrittura (sospendere o disattivare la azzera), e `velia.accoda_agenti_pianificati()` — schedulata ogni minuto come `velia-agenti-tick`, verificata dal vivo col ruolo del cron — crea esecuzione `pianificata` + job nella coda unica con `for update skip locked`; un tick perso non perde nulla (al giro dopo è ancora scaduta). Il worker resta uno.
- **Limiti due volte (RF-E-09)**: sul tenant come quelli dello spazio (`limite_agenti_attivi` 5, `limite_esecuzioni_concorrenti` 2, `frequenza_minima_agenti` — i numeri sono commerciali, §6.9); esposti da `GET /api/agenti/limiti`, imposti dalle rotte — 409 `LIMITE_AGENTI` su creazione/riattivazione oltre soglia, **429 `LIMITE_ESECUZIONI` con `ritentaTraSecondi: 20`**, 409 `LIMITE_FREQUENZA` su pianificazioni più fitte del piano (aggiunta onesta: il mock non lo copriva). Le pianificate non passano dal 429: consumano dal piano e il worker le serializza comunque.
- **Contratto del mock alla lettera**: fonti idratate in lettura (etichette pronte: «Archivio Privato — tutto», compagnia/ramo per nome, «non più disponibile» per i documenti spariti) e nude in scrittura; 400 `AGENTE_INCOMPLETO`/`PARAMETRI_MANCANTI`/`PARAMETRO_NON_VALIDO`, 409 `AGENTE_DISATTIVO`; duplica = copia **disattiva** con pianificazione sospesa e senza storico; elenco Paginato attivi-prima con `ultimaEsecuzione`; predefiniti (RF-E-10) da `dati/agenti-predefiniti.json`, dato di piattaforma già idratato.
- **Il documento su template (RF-E-13) si genera on-demand dallo storico**: l'esecuzione fotografa il `template_output_id` alla conclusione (l'agente può cambiare template dopo), `GET …/documento` impagina output+citazioni con la Fase 4 (identità visiva, template del tenant compresi), nome file `<agente>-<esecuzione>.<formato>`; `documentoGeneratoUrl` compare solo su completate col template.
- Migrazione `20260826190000_agenti.sql` (tabelle, tick, `create extension pg_cron`, limiti sul tenant). Test: `test/agenti.spec.ts` (9: schemi, prompt, rotte prima del db) e `test/integrazione-agenti.spec.ts` (10, tenant di collaudo, motore finto a copione: fonti idratate e prossima occorrenza, 409/429 coi limiti veri, esecuzione→job→esito con log e documento DOCX riletto, RF-E-08, retry fino al fallimento persistente, tick chiamato davvero, duplica spenta, cascata del DELETE). Suite completa 175/175. **La suite d'integrazione pretende il worker dev FERMO** anche qui: la coda è una, e un worker acceso ruba i job ai test (successo, coi centesimi del motore vero a farlo notare).
- Rimandati con motivo: canali di notifica oltre l'in-app (email, §6.10 — il polling dello storico è il canale, come da contratto FE); annullamento di un'esecuzione in corso (nessun contratto FE); formato `tabella` come struttura dati (oggi è l'esito in tabella Markdown, RF-E-02 soddisfatto — celle tipizzate quando servirà il lookup §7).

### ✅ Fase 8 — Memoria — **completata** (24/08/2026) · RF-G-01…G-06

`/api/ricordi` risponde dal backend (proxy al 3002); il mock resta per la demo. La memoria si alimenta **solo imparando**: nessuna `POST`, il contratto è `GET`/`PATCH`/`DELETE` come nel FE. Collaudata dal vivo sul tenant demo (prima versione con Haiku e cron: 17 ricordi da 5 conversazioni, 5 candidati scartati dal perimetro per dati sanitari, doppioni fermati; poi il committente ha chiesto **apprendimento durante la conversazione e lo stesso motore della chat**, ed è così che è chiusa). Decisioni da sapere:

- **La memoria impara a ogni risposta, come Claude, e si vede**: a messaggio dell'assistente persistito, il gestore d'interrogazione — prima del `fine` — emette l'attività «Cerco qualcosa da ricordare», chiama `apprendi()` (`worker/memoria/gestore.ts`) e, se ha imparato qualcosa, l'evento di stream **`memoria`** `{ ricordi: RicordoAppreso[] }` (aggiunta additiva al contratto, in `contratto/conversazioni.ts` e `core/models/conversazione.ts`; `memoria-saltata` negli eventi del job se l'estrazione fallisce: non è un errore della risposta). Il FE (`chat-store` → `ricordiAppresi` sul messaggio in stream, bolla con il blocco «Memorizzato» che porta a `/memoria`) lo mostra sotto la risposta; il mock lo emette nello scenario delle franchigie. Nessun tick d'accodamento, nessun «ferma da N minuti»; il job `memoria` a sé resta per rilavorare una conversazione a mano. La contabilità sta in **`velia.apprendimenti.appresa_fino_a`**, fuori da `conversazioni` (il cui `updated_at` ordina lo storico del FE e non si tocca): si leggono solo gli scambi oltre il cursore, e il cursore **avanza solo quando si impara** — senza risposta dell'assistente non si muove, così la domanda resta nel contesto del giro dopo. Idempotente: un secondo giro trova zero scambi nuovi e non chiama il motore. Costo: con Opus ~5 ¢ a risposta in più (misurato dal vivo: 0,048 $).
- **Lo stesso motore switch-abile della chat e delle tabelle**: `EstrattoreMotore` apre una sessione `MotoreAgentSdk` senza documenti (directory vuota: le mura restano quelle) con le istruzioni d'estrazione come prompt di sistema e `tenant.modello_motore` come modello (RF-D-02: lo switch da Impostazioni vale anche qui; default `MODELLO_MOTORE` = Opus 5); `maxTurni` 4, costo dal SDK in `consumi` (origine `app`). Niente Haiku.
- **Gli istanti viaggiano come testo** fra Postgres e Node (`inviato_il::text`, `$2::timestamptz`): un `Date` di JS ha i millisecondi, Postgres i microsecondi, e l'ultima risposta risultava sempre «nuova» (riappresa a ogni giro finché non l'ha detto `consumi`).
- **Il trigger `tocca_updated_at` ora rispetta un valore esplicito** (`if new.updated_at is not distinct from old.updated_at`): prima riscriveva sempre `now()`, e nessuno (retention, test, API) poteva dire «questa riga è vecchia». Vale per tutte le tabelle che lo usano.
- Job `memoria` (`worker/memoria/{estrattore,perimetro,gestore}.ts`): il modello riceve gli scambi non ancora appresi e i ricordi già noti e propone un array JSON (max 3 candidati, «meglio nessuno che uno debole», mai «L'utente…» come soggetto, mai cosa contiene l'archivio, nessuno strumento); **perimetro GDPR nel prompt E nel validatore** (`valutaPerimetro`: art. 9 e art. 10 per parole chiave sulla persona — «salute», «malattia», «invalidità» sono anche rami e passano; dati eccedenti: CF, IBAN, email, telefono, nascita, indirizzo, targa, credenziali; 20–400 caratteri); doppioni per **impronta** (`velia.ricordi.impronta`, minuscolo e spazi compressi); l'**ambito lo decide il server** (`ambitoEffettivo`: una preferenza è sempre personale dell'autore). Il testo di un candidato scartato non resta da nessuna parte: negli eventi del job va solo il motivo. Consumi con origine `app`, modello Haiku e costo di listino.
- Categorie allineate al contratto FE (`prassi|cliente|preferenza|decisione|altro`; `fatto` → `altro`). RLS di scrittura: modifica e cancellazione su tenant + propri personali; il `with check` pretende `utente_id` nullo per `tenant` e uguale a sé per `personale` — spostare a personale = **prenderlo in carico**, spostare a tenant = condividere. Con la RLS in sessione «non visibile» e «inesistente» sono lo stesso 404.
- **Retention: meccanismo senza politica** (RNF-03, punto aperto 8): `tenant.memoria_retention_giorni` (null = nessuna scadenza) e `tenant.memoria_attiva` (interruttore: spento, la chat non accoda e un job eventuale si chiude senza leggere); `velia-memoria-tick` (pg_cron, ogni notte alle 3:17) chiama solo `scada_ricordi()`, che cancella davvero (su `updated_at`: un ricordo corretto di recente è vivo). Nessuna rotta per queste due colonne: le imposta il committente finché il FE non le espone.
- RF-G-06 era già soddisfatto dalla Fase 3/7: `caricaDna` porta i ricordi attivi (tenant + personali dell'utente) nel prompt di chat e agenti.
- Migrazioni `20260827090000_memoria.sql`, `20260827090100_memoria_tick.sql`, `20260827110000_memoria_in_linea.sql` (le prime due portano ancora il tick d'accodamento e l'attesa; la terza li toglie). Test: `test/memoria.spec.ts` (14: perimetro, interprete, impronta, rotte prima del db), `test/integrazione-memoria.spec.ts` (7, tenant di collaudo, estrattore finto: job senza risposta che non avanza, job con perimetro, doppioni, modello del tenant e consumi, solo scambi nuovi al giro dopo, RF-G-02, RF-G-03 con spostamento in entrambi i versi, cancellazione effettiva, retention, memoria spenta) e l'accodamento a fine risposta in `integrazione-conversazioni` (che stubba `gestori.memoria`: non impara). Suite completa 196/196. **Trappola**: `lavoraTutto` dei test pesca qualunque job in coda, anche di altri tenant — il gestore nel test ignora i job fuori dal tenant di collaudo (una run aveva scritto due fixture nella memoria del tenant demo: rimosse).
- **Fornitori terzi Anthropic-compatibili (RF-D-03, 24/08)**: `worker/motore/fornitori.ts` — un modello del catalogo con `fornitore: 'hostyourai'` (GLM 5.2 `zai-org/GLM-5.2`, Kimi K3 `moonshotai/Kimi-K3`, datacenter UE) gira nella STESSA sessione Agent SDK con `Options.env` = ambiente del processo + `ANTHROPIC_BASE_URL=https://hostyourai.com` + `ANTHROPIC_API_KEY=hyai-…` (senza token OAuth), niente `effort`, costo in `consumi` a tariffa del catalogo (`tariffaUsdPerMilione`, l'SDK non conosce il listino). Chiavi in `.env` (`HOSTYOURAI_API_KEY`, `HOSTYOURAI_BASE_URL`); senza chiave le voci restano non selezionabili (`catalogoModelli`, 409 col motivo). Vale per chat, tabelle, agenti e memoria; titoli/suggerimenti/classificazione restano su Haiku diretto. **Collaudati col motore agentico vero** (`tools/collaudo-motore.ts "<domanda>" zai-org/GLM-5.2`): GLM 5.2 → 7 turni, 20,6 s, 5/5 citazioni verificate, 0,004 $; Kimi K3 → 9 turni, 42,7 s, 4/4 citazioni verificate, 0,007 $ — stessa risposta corretta di Opus sulla franchigia cristalli. Limite noto: il gateway non riporta i token di input in streaming (`in 0`), quindi il costo a tariffa è sottostimato; i modelli «pensano» prima di scrivere (con `max_tokens` bassi il testo torna vuoto: non è un problema nell'SDK, che non lo limita). Test: `test/fornitori.spec.ts`.
- Rimandati con motivo: rotte per attesa/retention/interruttore (nessun contratto FE); memoria dalle esecuzioni degli agenti (`origineConversazioneId` è una conversazione; un agente non ha un utente che «dica» prassi); dedup semantico oltre l'impronta (il prompt riceve i noti, basta per ora).

### ✅ Ancoraggio delle citazioni e Archivio Pubblico in sessione — **fatto** (25/08/2026)
...
la sezione Impostazioni → Crediti, non c'è più il calcolo durante le chat (né
per tabelle, agenti e conversioni), non ci sono più il blocco a saldo esaurito
(`429 CREDITI_ESAURITI`) e il «tipico» per modello nel catalogo. Rimossi
`api/crediti/rotte.ts`, `contratto/crediti.ts`, `worker/crediti.ts`,
`test/integrazione-crediti.spec.ts`, `mocks/crediti.mjs` e la pagina Angular.

**Resta la misura del lavoro**: `velia.consumi` continua a registrare token e
costo di ogni sessione (`registraConsumi`), che è ciò che serve per l'analisi
dei costi. **Restano anche gli oggetti a database** (`velia.crediti_pesi`,
`velia.crediti_movimenti`, `velia.saldo_crediti`, `tenant.crediti_inclusi`):
nessuno li legge più, non hanno cron né trigger, e cancellarli butterebbe via
lo storico dei movimenti — si fa con una migrazione dedicata quando si decide
che quello storico non serve. **Restano i limiti di piano degli agenti**
(RF-E-09: agenti attivi, esecuzioni concorrenti, frequenza minima): sono un
tetto d'uso, non un prezzo.

Sotto, com'era finché c'era.

Decisione del committente: **59 €/mese con 600 crediti inclusi, pacchetti a parte (1.000 / 3.000 / 10.000), Opus come qualità di default**. L'unità è il credito, non la risposta né il token, e l'addebito è **proporzionale al lavoro fatto**: 1 credito ogni 4 centesimi di calcolo della sessione (`crediti_pesi.per_usd = 25`), minimo 1 — un «ciao» costa 1, un confronto documentale con Opus ~10, con Sonnet ~5, con un modello open 1-2; la conversione di un documento caricato 1 fisso; memoria, titoli e suggerimenti inclusi. I pesi per classe (`opus 10 / sonnet 5 / haiku 3 / open 2`) restano come «tipico» nel catalogo (`ModelloAI.creditiPerRisposta`) e come riserva se una sessione non riporta il costo.

- Migrazioni `20260827150000_crediti.sql` (`tenant.crediti_inclusi`, `velia.crediti_pesi`, `velia.crediti_movimenti` con `pacchetto|rettifica|addebito`, `velia.saldo_crediti(tid)`: gli inclusi del mese si consumano per primi e scadono, i pacchetti dopo e non scadono — l'eccedenza mensile oltre gli inclusi è ciò che intacca i pacchetti) e `20260827160000_crediti_proporzionali.sql`.
- Addebito nel worker (`worker/crediti.ts`, `addebitaCrediti` con `costoUsd` dell'esito): chat a risposta scritta, tabelle riga per riga, agenti a esecuzione completata, conversione a documento pronto (solo tenant). Un addebito fallito non fa fallire il job (log).
- Porte: `richiediCrediti` → **429 `CREDITI_ESAURITI`** con messaggio «ricarica» all'invio in chat, a ogni mutazione di tabella che genera, all'esecuzione manuale di un agente. Un job già partito si completa. Il FE lo mostra come «Crediti esauriti», non come «riprova fra poco».
- `GET /api/crediti` (saldo, listino, mese per operazione, ultimi 100 movimenti); pagina Impostazioni → Crediti (saldo grande, barra degli inclusi, riserva sotto il 20%, «quanto pesa», movimenti); mock `mocks/crediti.mjs`. **Pacchetti accreditati a mano** (`insert into velia.crediti_movimenti … 'pacchetto'`) finché non c'è Stripe.
- **Misura, non stima** (25/08, «mi sembra spannometrica»): crediti con **un decimale** (`numeric(12,1)`, minimo 0,1 solo se c'è stato lavoro), e ogni addebito registra **token letti, scritti e costo** (`token_input` = input + cache, `token_output`, `costo_usd`) mostrati nel registro. I token si contano **dallo stream, turno per turno** (`message_start` per input e cache, `message_delta` per l'output), e vincono sul totale dell'SDK quando vedono di più. **HostYourAI manda `input_tokens: 0` ovunque** (verificato sugli eventi grezzi): per quei modelli l'input si **stima dal contesto ricevuto** (prompt + risposte + risultati dei tool, ~3,6 caratteri/token) e l'addebito lo dichiara (`token_stimati`, «(stimati)» nel FE). Collaudo GLM: da «in 0» a 18.395 stimati + 2.004 scritti → 0,035 $ → 0,9 crediti. Migrazioni `20260827170000_crediti_precisi.sql`, `20260827180000_crediti_stima.sql`.
- Test: `test/integrazione-crediti.spec.ts` (3: saldo e catalogo, addebito proporzionale a un decimale con token registrati e riserva per classe, eccedenza sui pacchetti e 429). Il tenant demo è partito da 600 il 25/08.
...
| 8 | **Politica di retention** di memoria e audit (RNF-03, RF-G-05) — il meccanismo c'è (Fase 8: `tenant.memoria_retention_giorni`, null = nessuna scadenza) | Fase 8 | Meccanismo senza politica: rischio di conservare troppo |
