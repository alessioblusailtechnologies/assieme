...
- **La memoria impara a ogni risposta, come Claude**: a messaggio dell'assistente persistito, il gestore d'interrogazione accoda un job `memoria` per la conversazione (`memoria-saltata` negli eventi se l'accodamento fallisce: non è un errore della risposta). Nessun tick d'accodamento, nessun «ferma da N minuti». La contabilità sta in **`velia.apprendimenti.appresa_fino_a`**, fuori da `conversazioni` (il cui `updated_at` ordina lo storico del FE e non si tocca): il job legge solo gli scambi oltre il cursore, e il cursore **avanza solo quando si impara** — un job senza risposta dell'assistente non lo muove, così la domanda resta nel contesto del giro dopo. Idempotente: un doppio arrivo trova zero scambi nuovi e non chiama il motore.
