...
- **Gli ingressi**: dal pulsante, `POST …/messaggi` con `esportazione: { formato, templateId?, messaggioId?, istruzioni? }` (contratto additivo; il job `interrogazione` col campo produce un documento, non una risposta: niente blocco citazioni, messaggio assistente = nota del motore documentale + `documenti`); a parole, il tool `esportazione_elaborata` della chat esegue la stessa funzione in linea e i file consegnati diventano documenti della risposta. Crediti: addebito come `risposta` con descrizione «Esportazione elaborata: …». File orfani cancellati se la risposta non si salva. Config: `SANDBOX_AVVIATORE` (docker|fly, vuoto = non disponibile), `SANDBOX_IMMAGINE`, `FLY_API_TOKEN`, `FLY_APP_SANDBOX`, `FLY_REGIONE`.
- **Collaudo col motore vero e la sandbox Docker** (`tools/collaudo-elaborata.ts`, tenant demo senza template): «scheda di due pagine per il cliente Rossi sulle garanzie Incendio e Furto di Km&Servizi, con tabella e fonti» → il modello ha letto CGA, DIP e DIP Aggiuntivo, scritto HTML+CSS, stampato con Chromium, reso le pagine in PNG e le ha guardate, **compattato il testo per stare in due pagine in tre giri**, consegnato un PDF di 115 KB davvero professionale (testata, tabella, fonti con articoli e pagine, nota di edizione). 32 turni, 326 s, 1,60 $. È il livello di qualità voluto; il costo è quello di una sessione lunga, coerente col pricing a crediti.
...
### ✅ Crediti (pricing) — **fatto** (25/08/2026), fuori dalle fasi
