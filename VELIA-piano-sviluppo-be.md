...
### Fase 6 — Impostazioni complete · ~5 giorni · RF-D-01…D-04, D-06/07, D-14/16, utenti — **in corso**

**Primo pezzo fatto (24/08/2026): `/api/modelli` dal backend.** Il catalogo dice la verità sul motore: l'attivo (`GET /api/modelli/attivo`) è derivato da `MODELLO_MOTORE` — oggi **Claude Opus 5**, quello con cui chat e tabelle sono collaudate — mai una preferenza salvata che il worker ignorerebbe. Le schede (RF-D-03) sono configurazione nel codice (`contratto/modelli.ts`, l'id SDK non esce dall'API); Sonnet 5, Haiku 4.5, GPT-5.2 e Mistral sono in catalogo **non disponibili** finché la scelta per agenzia non è cablata fino al job — così il PUT tiene i codici del mock (404, 409 `NON_DISPONIBILE`, 403 operatore) senza fingere un cambio che non avverrebbe; confermare l'attivo è idempotente e lascia la voce nello storico (oggetto `modello`, `registraStorico` generalizzato). Fixture del mock allineata (Opus 5 in testa, provider terzi non disponibili). Proxy `/api/modelli` → 3002. Test: `test/modelli.spec.ts` (5) + `test/integrazione-modelli.spec.ts` (1, voce di storico sul tenant di collaudo). Quando la scelta diventerà reale: modello per job (l'astrazione qui sotto), colonna di tenant, `disponibile: true` sugli Anthropic alternativi.

Resta da fare:

- Modello e provider (RF-D-02): l'astrazione `motore/` formalizzata — interfaccia unica, prima implementazione Anthropic con scelta del modello portata fino al job (oggi il modello è fissato da `MODELLO_MOTORE`/`MODELLO_TABELLE` a livello di processo)
