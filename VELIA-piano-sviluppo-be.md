...
`/api/ricordi` risponde dal backend (proxy al 3002); il mock resta per la demo. La memoria si alimenta **solo imparando**: nessuna `POST`, il contratto è `GET`/`PATCH`/`DELETE` come nel FE. **Collaudata dal vivo** sul tenant demo (il cron ha lavorato le conversazioni reali con Haiku: 17 ricordi da 5 conversazioni, 5 candidati scartati dal perimetro per dati sanitari, doppioni fermati, ~0,2 ¢ a job). Decisioni da sapere:

- **«Fine conversazione» la decide il tick**, non il client: `velia-memoria-tick` (pg_cron, ogni 5 minuti) chiama `accoda_apprendimento()` — una conversazione è conclusa quando **nessun messaggio** da almeno `tenant.memoria_attesa_minuti` (default 15) e c'è almeno una risposta dell'assistente oltre `appresa_fino_a`. La contabilità sta in **`velia.apprendimenti`** (`appresa_fino_a`, `accodato_il`), fuori da `conversazioni`: né il tick né il job toccano la riga della conversazione, il cui `updated_at` ordina lo storico del FE. Un accodamento più vecchio di un giorno (job perso) si riaccoda: il job rilegge lo stato ed è idempotente.
- **Gli istanti viaggiano come testo** fra Postgres e Node (`inviato_il::text`, `$2::timestamptz`): un `Date` di JS ha i millisecondi, Postgres i microsecondi, e l'ultima risposta risultava sempre «nuova» — riaccodata ogni 5 minuti finché non l'ha detto `consumi` (20 job per 5 conversazioni).
...
- **Retention: meccanismo senza politica** (RNF-03, punto aperto 8): `tenant.memoria_retention_giorni` (null = nessuna scadenza) e `tenant.memoria_attiva` (interruttore); `scada_ricordi()` nello stesso tick cancella davvero (su `updated_at`: un ricordo corretto di recente è vivo). Nessuna rotta per queste tre colonne: le imposta il committente finché il FE non le espone.
...
- Migrazioni `20260827090000_memoria.sql` e `20260827090100_memoria_tick.sql`. Test: `test/memoria.spec.ts` (14: perimetro, interprete, impronta, rotte prima del db) e `test/integrazione-memoria.spec.ts` (7, tenant di collaudo, estrattore finto: tick selettivo, job con perimetro e doppioni, consumi, RF-G-02, RF-G-03 con spostamento in entrambi i versi, cancellazione effettiva, retention, memoria spenta). Suite completa 196/196. **Trappola nuova**: il cron accoda anche le conversazioni ferme degli altri tenant e `lavoraTutto` dei test le pescherebbe con l'estrattore finto — il gestore nel test ignora i job fuori dal tenant di collaudo (una run aveva scritto due fixture nella memoria del tenant demo: rimosse).
