...
export interface OpzioniTrascrizione {
  ...
  /**
   * Quanto aspettare Voxtral. La dettatura è breve e resta a un minuto; un
   * audio o un video caricato come documento (11/09/2026) può durare molto
   * di più, e la sua lettura gira nel worker, dove si può aspettare.
   */
  tempoMassimoMs?: number | undefined;
}
...
export class TrascrittoreMistral implements Trascrittore {
  ...
  async trascrivi(audio: AudioDaTrascrivere, opzioni: OpzioniTrascrizione = {}): Promise<string> {
    ...
    const tempo = opzioni.tempoMassimoMs ?? 60_000;
    let risposta = await this.chiama(audio, true, tempo);
    if (!risposta.ok && risposta.status >= 400 && risposta.status < 500) {
      ...
      if (/context.?bias/i.test(dettaglio)) {
        ...
        risposta = await this.chiama(audio, false, tempo);
  ...
  private chiama(audio: AudioDaTrascrivere, conTermini: boolean, tempoMassimoMs: number): Promise<Response> {
    ...
    return fetch(ENDPOINT, {
      ...
      body: form,
      signal: AbortSignal.timeout(tempoMassimoMs),
    });
  }
}
