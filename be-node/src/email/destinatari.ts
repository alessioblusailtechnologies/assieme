 ...
 *
 * Lo usano lo strumento `prepara_email` della chat e il piano di un agente:
 * in tutti e due i casi l'indirizzo **non lo sceglie il modello**. Viene
 * dall'anagrafica o dalle parole dell'utente, e quando non si trova non si
 * indovina: il motivo torna indietro in due forme, `motivo` per il modello
 * che deve girarlo all'utente, `perUtente` per la scheda del piano che lo
 * mostra direttamente.
 */

export type EsitoDestinatario =
  | { esito: 'trovato'; destinatario: DestinatarioBozza }
  | { esito: 'non-trovato'; motivo: string; perUtente: string };
...
export async function risolviDestinatario(
  ...
  if (!testo) return { esito: 'non-trovato', motivo: 'Manca il destinatario.', perUtente: 'Manca il destinatario.' };
  ...
  if (/^(me|a me|io|me stess[oa])$/i.test(testo)) {
    ...
    if (!utente?.email) {
      return {
        esito: 'non-trovato',
        motivo: 'Non trovo l’indirizzo email dell’utente: chiediglielo.',
        perUtente: 'Il tuo profilo non ha un indirizzo email.',
      };
    }
    return {
      esito: 'trovato',
  ...
  if (colleghi.rows.length > 1) {
    return {
      ...
      perUtente: `Nell’agenzia c’è più di un collega che si chiama «${testo}»: scrivi il suo indirizzo.`,
    };
  }
  ...
  if (esito.esito === 'ambiguo') {
    const nomi = esito.candidati.map((c) => `«${c.nome}»`).join(', ');
    return {
      esito: 'non-trovato',
      motivo: `Più clienti somigliano a «${testo}»: ${nomi}. Chiedi all’utente quale, invece di sceglierne uno.`,
      perUtente: `Più clienti somigliano a «${testo}» (${nomi}): referenzia quello giusto con @.`,
    };
  }
  if (esito.esito === 'assente') {
    return {
      ...
      perUtente: `«${testo}» non è un indirizzo email, né un collega, né un cliente in anagrafica.`,
    };
  }
  const { cliente } = esito;
  ...
  if (!indirizzo) {
    return {
      ...
      perUtente: `${cliente.nome} è in anagrafica senza indirizzo email: aggiungilo alla sua scheda, o scrivi l’indirizzo nella richiesta.`,
    };
  }
