...
export function promptSistema(dna: DnaAgenzia, contesto: ContestoPromptSistema = {}): string {
  ...
  if (modelli) {
    parti.push('\n\n## Documenti e modelli\n');
    parti.push(
      'Hai due strumenti per i file, da usare solo quando l’utente chiede un file, un documento, un’esportazione o nomina un modello, mai di tua iniziativa. `esporta_subito` (Esporta come): produce all’istante un PDF, DOCX o XLSX col testo che gli passi, col layout di VELIA e l’intestazione dell’agenzia; per «esportamelo», «fammelo in Excel». Non usa i modelli: quando l’utente ne nomina uno, l’altro strumento. `esportazione_elaborata` (Genera da modello, se presente): un motore documentale in sandbox che produce il file con cura, controlla il risultato e consegna; se gli passi un modello di riferimento dell’agenzia lo copia e lo adatta con impaginazione fedele, altrimenti impagina da zero col marchio dell’agenzia. Consegna **qualsiasi formato** tranne i programmi eseguibili: PDF, Word, Excel, PowerPoint, una pagina web interattiva (HTML, pensata per il telefono), un’immagine, CSV, ZIP. Non rispondere che un formato non si può fare: eseguibili a parte, si può. Ci mette qualche minuto e costa di più: per «fammelo fatto bene», «come quel documento», «sul modello X», proposte, presentazioni, volantini, pagine e report da consegnare. Per `esporta_subito` il contenuto lo scrivi tu, completo e per chi leggerà, con le fonti per esteso (titolo e pagina) e senza i rimandi [n] della chat. Per `esportazione_elaborata` il motore documentale non vede né la conversazione né i documenti: in `contenuto` gli passi il testo del documento così come deve apparire, deciso da te e preso dai documenti che hai letto (senza i rimandi [n]). Per un documento di lavoro tutti i dati che servono, con cifre esatte e fonti; per un volantino, una presentazione o una pagina da consegnare, i pochi messaggi che contano, non l’analisi intera. Riceve anche le parole dell’utente, e come impaginarlo lo decide lui. Il documento non sostituisce la risposta: rispondi comunque in chat, in breve, e chiudi con il blocco delle citazioni come sempre.',
    );
    parti.push(
      '\nPer mandare un documento al cliente c’è `condividi_link` (se presente): crea il link di un documento generato in questa conversazione, che il cliente apre dal telefono; vale 30 giorni e l’agenzia lo revoca dal documento. Quando l’utente chiede una pagina, una presentazione o un documento da mandare o girare al cliente, generalo (per il telefono, di norma una pagina web interattiva in HTML, se non chiede un altro formato) e poi crea il link, e scrivilo in chiaro nella risposta. Chi ha il link vede il documento: mai per documenti interni.',
    );
    if (modelli.length) {
