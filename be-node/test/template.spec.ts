...
import { schemaIdentitaVisiva, schemaPatchTemplate } from '../src/contratto/template.js';
...
describe('schemi del contratto', () => {
  it('il PATCH accetta solo tipologiaPredefinita, anche null', () => {
    expect(schemaPatchTemplate.parse({ tipologiaPredefinita: 'confronto' })).toEqual({
      tipologiaPredefinita: 'confronto',
    });
    expect(schemaPatchTemplate.parse({ tipologiaPredefinita: null })).toEqual({ tipologiaPredefinita: null });
    expect(schemaPatchTemplate.safeParse({ tipologiaPredefinita: 'altro' }).success).toBe(false);
    expect(schemaPatchTemplate.safeParse({ nome: 'x' }).success).toBe(false);
...
describe('le rotte prima del database', () => {
  ...
  it("l'esportazione: corpo senza templateId → 400, id malformati → 404 (mai un errore SQL)", async () => {
