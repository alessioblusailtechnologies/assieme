...
- **Non-copertura senza banner** (08/2026, su indicazione del committente): la dichiarazione di RF-C-08 vive nel testo della risposta, non in una fascia sopra il messaggio. L'evento `non-supportato` resta nel contratto SSE: al backend serve, e un trattamento grafico futuro non richiederà una modifica di specifica.
...
- Restano al backend, con motivo: l'apprendimento automatico vero (RF-G-01 — le fixture lo rappresentano), la memoria dentro le esecuzioni degli agenti (RF-G-06), la politica di retention configurabile (RF-G-05), il server MCP con i suoi tool e i conteggi (RF-F-01/03).
